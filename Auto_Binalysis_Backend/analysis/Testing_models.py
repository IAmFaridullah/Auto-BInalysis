...
def test_models(dataset, model_dir):
    ...
    if dataset_name == 'MemberChurnTest':
        ...
        df['GoldenCard'] = df['GoldenCard'].map(
            {'Yes': 1, 'No': -1, 'No Silver Card': 0})
        ...
        df = pd.read_excel(dataset)
        # Convert the categorical variable to numerical values
        df['RecentCardRenewal'] = df['RecentCardRenewal'].map(
            {'Yes': 1, 'No': 0})
        ...
        for col in predictions.columns[1:]:
            ...
            forecast = loaded_model.predict(
                start=len(predictions), end=len(predictions)+51)
        ...
        mapping = df[['TransactionNo', 'Cluster']].drop_duplicates(
        ).set_index('TransactionNo')['Cluster'].to_dict()

        # Group products based on their invoice numbers
        grouped_df = df.groupby(['TransactionNo'])[
            'Items'].apply(list).reset_index()
        ...
        # Create a mapping between invoice numbers and their respective clusters
        mapping = df[['OrderNo', 'Cluster']].drop_duplicates().set_index('OrderNo')[
            'Cluster'].to_dict()
        ...
        # Create a mapping between invoice numbers and their respective clusters
        mapping = df[['Member_number', 'Cluster']].drop_duplicates(
        ).set_index('Member_number')['Cluster'].to_dict()

        # Group products based on their invoice numbers
        grouped_df = df.groupby(['Member_number'])[
            'item'].apply(list).reset_index()
        ...
        # Create a mapping between invoice numbers and their respective clusters
        mapping = df[['Inv No', 'Cluster']].drop_duplicates().set_index('Inv No')[
            'Cluster'].to_dict()

        # Group products based on their invoice numbers
        grouped_df = df.groupby(['Inv No'])[
            'Product Name'].apply(list).reset_index()
        ...
        # Create a mapping between invoice numbers and their respective clusters
        mapping = df[['Inv #', 'Cluster']].drop_duplicates().set_index('Inv #')[
            'Cluster'].to_dict()

        # Group products based on their invoice numbers
        grouped_df = df.groupby(['Inv #'])[
            'Product Name'].apply(list).reset_index()
        ...
        if set(df['PurchaseInEveryMonth'].dropna().unique()) == {'Yes', 'No'}:
            df['PurchaseInEveryMonth'] = df['PurchaseInEveryMonth'].map(
                {'Yes': 1, 'No': 0})
        ...
        with open(model_dir, 'rb') as f:
            ...
            df['Product_Class'] = le.inverse_transform(
                df['Product_Class_Encoded'])
